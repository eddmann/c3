...
// clang-format off
static constexpr std::array<std::uint64_t, 781> POLYGLOT_RANDOM = {
...
// Index offsets in POLYGLOT_RANDOM array
static constexpr std::size_t POLYGLOT_PIECE_OFFSET = 0;
static constexpr std::size_t POLYGLOT_CASTLE_OFFSET = 768;
static constexpr std::size_t POLYGLOT_ENPASSANT_OFFSET = 772;
static constexpr std::size_t POLYGLOT_TURN_OFFSET = 780;

// Polyglot piece index: kind*2 + color (kind: P=0,N=1,B=2,R=3,Q=4,K=5; color: black=0,white=1)
// Engine piece order: WP(0),WN(1),WB(2),WR(3),WQ(4),WK(5),BP(6),BN(7),BB(8),BR(9),BQ(10),BK(11)
static constexpr std::array<std::size_t, 12> ENGINE_TO_POLYGLOT_PIECE = {
    1,  // WP -> white pawn  (kind=0, color=1) = 0*2+1 = 1
    3,  // WN -> white knight (kind=1, color=1) = 1*2+1 = 3
    5,  // WB -> white bishop (kind=2, color=1) = 2*2+1 = 5
    7,  // WR -> white rook  (kind=3, color=1) = 3*2+1 = 7
    9,  // WQ -> white queen (kind=4, color=1) = 4*2+1 = 9
    11, // WK -> white king  (kind=5, color=1) = 5*2+1 = 11
    0,  // BP -> black pawn  (kind=0, color=0) = 0*2+0 = 0
    2,  // BN -> black knight (kind=1, color=0) = 1*2+0 = 2
    4,  // BB -> black bishop (kind=2, color=0) = 2*2+0 = 4
    6,  // BR -> black rook  (kind=3, color=0) = 3*2+0 = 6
    8,  // BQ -> black queen (kind=4, color=0) = 4*2+0 = 8
    10, // BK -> black king  (kind=5, color=0) = 5*2+0 = 10
};
...
std::uint64_t OpeningBook::compute_polyglot_key(const Position& pos) {
  ...
  for (std::uint8_t sq = 0; sq < 64; ++sq) {
    const Square square = Square::from_index(sq);
    const auto piece = pos.board.piece_at(square);
    if (piece.has_value()) {
      const std::size_t polyglot_piece = ENGINE_TO_POLYGLOT_PIECE[static_cast<std::size_t>(*piece)];
      key ^= POLYGLOT_RANDOM[POLYGLOT_PIECE_OFFSET + polyglot_piece * 64 + sq];
    }
  }

  // Hash castling rights
  // Polyglot order: [0]=white O-O, [1]=white O-O-O, [2]=black O-O, [3]=black O-O-O
  if (pos.castling_rights.has(CastlingRight::WhiteKing)) {
    key ^= POLYGLOT_RANDOM[POLYGLOT_CASTLE_OFFSET + 0];
  }
  if (pos.castling_rights.has(CastlingRight::WhiteQueen)) {
    key ^= POLYGLOT_RANDOM[POLYGLOT_CASTLE_OFFSET + 1];
  }
  if (pos.castling_rights.has(CastlingRight::BlackKing)) {
    key ^= POLYGLOT_RANDOM[POLYGLOT_CASTLE_OFFSET + 2];
  }
  if (pos.castling_rights.has(CastlingRight::BlackQueen)) {
    key ^= POLYGLOT_RANDOM[POLYGLOT_CASTLE_OFFSET + 3];
  }

  // Hash en passant file (only if a pawn can actually capture)
  if (pos.en_passant_square.has_value()) {
    const Square ep_sq = *pos.en_passant_square;
    const std::uint8_t ep_file = ep_sq.file();

    // Check if a friendly pawn can actually capture en passant
    const Colour side = pos.colour_to_move;
    bool can_capture = false;

    // Check pawns on adjacent files that could capture
    const std::uint8_t ep_rank = ep_sq.rank();
    const std::uint8_t pawn_rank = (side == Colour::White) ? (ep_rank - 1) : (ep_rank + 1);

    if (ep_file > 0) {
      const Square left_sq = Square::from_file_and_rank(ep_file - 1, pawn_rank);
      if (pos.board.piece_at(left_sq) == pawn(side)) {
        can_capture = true;
      }
    }
    if (ep_file < 7 && !can_capture) {
      const Square right_sq = Square::from_file_and_rank(ep_file + 1, pawn_rank);
      if (pos.board.piece_at(right_sq) == pawn(side)) {
        can_capture = true;
      }
    }

    if (can_capture) {
      key ^= POLYGLOT_RANDOM[POLYGLOT_ENPASSANT_OFFSET + ep_file];
    }
  }

  // Hash side to move (Polyglot XORs when white to move)
  if (pos.colour_to_move == Colour::White) {
    key ^= POLYGLOT_RANDOM[POLYGLOT_TURN_OFFSET];
  }
...
// Convert big-endian bytes to host byte order
static std::uint64_t read_be64(const std::uint8_t* data) {
  return (static_cast<std::uint64_t>(data[0]) << 56) | (static_cast<std::uint64_t>(data[1]) << 48) |
...
bool OpeningBook::load(const std::filesystem::path& path) {
  ...
  if (file_size < 0 || static_cast<std::size_t>(file_size) % 16 != 0) {
    return false; // Invalid file size (must be multiple of 16 bytes)
  }

  const std::size_t entry_count = static_cast<std::size_t>(file_size) / 16;
  if (entry_count == 0) {
    loaded_ = true; // Empty book is valid
    return true;
  }
  ...
  for (std::size_t i = 0; i < entry_count; ++i) {
    const std::uint8_t* ptr = buffer.data() + i * 16;
    PolyglotEntry entry;
    entry.key = read_be64(ptr);
    entry.move = read_be16(ptr + 8);
    entry.weight = read_be16(ptr + 10);
    entry.learn = read_be32(ptr + 12);
    entries_.push_back(entry);
  }

  // Sort by key (usually already sorted, but ensure it)
...
std::vector<PolyglotEntry> OpeningBook::find_entries(std::uint64_t key) const {
  auto [begin, end] = std::equal_range(entries_.begin(), entries_.end(), key);
  return {begin, end};
}
...
std::optional<Move> OpeningBook::decode_polyglot_move(std::uint16_t encoded, const Position& pos) {
  ...
  const auto to_rank = static_cast<std::uint8_t>((encoded >> 3) & 0x7);
  const auto from_file = static_cast<std::uint8_t>((encoded >> 6) & 0x7);
  const auto from_rank = static_cast<std::uint8_t>((encoded >> 9) & 0x7);
  const auto promo = static_cast<std::uint8_t>((encoded >> 12) & 0x7);

  Square from = Square::from_file_and_rank(from_file, from_rank);
  Square to = Square::from_file_and_rank(to_file, to_rank);

  // Get the piece on the from square
  const auto piece = pos.board.piece_at(from);
  if (!piece.has_value()) {
    return std::nullopt; // No piece at source square
  }

  // Handle castling: Polyglot encodes as king-to-rook, we need king-to-final-square
  if (is_king(*piece)) {
    // White castling
  ...
  // Determine if this is en passant
  const bool is_en_passant =
      is_pawn(*piece) && pos.en_passant_square.has_value() && to == *pos.en_passant_square;

  // Determine captured piece
  std::optional<Piece> captured_piece;
  if (is_en_passant) {
    captured_piece = pawn(!colour(*piece));
  } else {
    captured_piece = pos.board.piece_at(to);
  }

  // Handle promotion
  std::optional<Piece> promotion_piece;
  if (promo > 0) {
    const Colour col = pos.colour_to_move;
    switch (promo) {
    case 1:
      promotion_piece = knight(col);
      break;
    case 2:
      promotion_piece = bishop(col);
      break;
    case 3:
      promotion_piece = rook(col);
      break;
    case 4:
      promotion_piece = queen(col);
      break;
    default:
      return std::nullopt; // Invalid promotion
    }
  }
...
std::optional<Move> OpeningBook::probe(const Position& pos) const {
  if (!loaded_) {
    return std::nullopt;
  }

  const std::uint64_t key = compute_polyglot_key(pos);
  ...
  // Use position's engine key as seed for deterministic selection per position
  const auto selected = select_weighted(entries, pos.key);
  if (!selected.has_value()) {
    return std::nullopt;
  }

  return decode_polyglot_move(selected->move, pos);
}

std::vector<std::pair<Move, std::uint16_t>> OpeningBook::probe_all(const Position& pos) const {
  std::vector<std::pair<Move, std::uint16_t>> result;
  ...
  for (const auto& entry : entries) {
    if (auto move = decode_polyglot_move(entry.move, pos)) {
      result.emplace_back(*move, entry.weight);
    }
  }
