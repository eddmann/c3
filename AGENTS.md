...
- Header highlights: `move_list.hpp` (fixed-capacity move container, no allocator on the hot path), `eval_terms.hpp` (piece values, PSQTs and the incremental accumulator the Board maintains), `pawns.hpp` (compile-time masks behind the pawn-structure terms).
...
- Measuring a search heuristic is an A/B question, and the engine exposes no runtime toggle for one (the only UCI option is `Hash`). Measure it the way the repo does: build both sides, compare `bench` node counts for the size of the tree, and settle strength with `scripts/compare_branches.py`. A local `#if` around the heuristic is a fine scaffold—do not ship it.
